...
import com.google.arcore.java.common.helpers.SnackbarHelper
import com.google.arcore.java.common.helpers.TapHelper
import com.google.arcore.kotlin.arcoredemo.ARCoreDemo
import com.google.arcore.kotlin.arcoredemo.R

/** Contains UI elements for Hello AR. */
class ARCoreView(val activity: ARCoreDemo) : DefaultLifecycleObserver {
                ...
                PopupMenu(activity, v).apply {
                    setOnMenuItemClickListener { item ->
                        when (item.itemId) {
                            ...
                            R.id.plane_settings -> launchPlaneSettingsMenuDialog()
                            else -> null
                        } != null
                    }
    ...
    private fun launchPlaneSettingsMenuDialog(){
        val session = session?: return

        val resources: Resources = activity.resources
        val strings = resources.getStringArray(R.array.plane_options_array)
        val checked = booleanArrayOf(activity.planeSettings.isVerticalPlaneEnabled)
        AlertDialog.Builder(activity)
            .setTitle("Plane Option")
            .setMultiChoiceItems(strings, checked) {_, which, isChecked -> checked[which] = isChecked}
            .setPositiveButton("Done") {_, _ ->
                val session = session ?: return@setPositiveButton
                activity.planeSettings.isVerticalPlaneEnabled = checked[0]
                activity.configureSession(session)
            }.show()
    }
}
