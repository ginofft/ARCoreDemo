...
import com.google.arcore.java.common.samplerender.Framebuffer
...
import com.google.arcore.java.common.samplerender.arcore.BackgroundRenderer
import com.google.arcore.java.common.samplerender.arcore.PlaneRenderer
import com.google.arcore.java.common.samplerender.arcore.SpecularCubemapFilter
import com.google.ar.core.exceptions.CameraNotAvailableException
import com.google.ar.core.exceptions.NotYetAvailableException
import com.google.arcore.java.common.helpers.DisplayRotationHelper
import com.google.arcore.java.common.helpers.TrackingStateHelper
import com.google.arcore.java.common.samplerender.SampleRender
import com.google.arcore.kotlin.helpers.LineRenderer
import com.google.arcore.kotlin.helpers.TextRenderer
import java.io.IOException
import java.nio.ByteBuffer
import java.nio.FloatBuffer
import kotlin.math.pow
import kotlin.math.sqrt
import com.google.arcore.java.common.MVBB.MVBB

/** Renders the HelloAR application using our example Renderer. */
class ARCoreRenderer(val activity: ARCoreDemo) :
    ...
    val viewProjectionMatrix  = FloatArray(16) // projection x view
...
//        minBoundingBox.setPnts(coordArray)
        ...
        for ((anchor1, _) in filteredWrappedAnchors){
            for ((anchor2,_) in filteredWrappedAnchors){
                if (!anchor1.equals(anchor2)){
                    var distance = calculateDistance(anchor1, anchor2)
                    var midPose = getMidPose(anchor1.pose, anchor2.pose)
                    textRenderer.draw(
                        render,
        ...
        val firstHitResult =
            hitResultList.firstOrNull { hit ->
                when (val trackable = hit.trackable!!) {
                    ...
                    is Plane -> PlaneRenderer.calculateDistanceToPlane(hit.hitPose, camera.pose) > 0
    ...
    private fun calculateDistance(anchor1: Anchor, anchor2: Anchor):Float {
        var pose1 = anchor1.pose
        var pose2 = anchor2.pose
        var distance = sqrt((pose1.tx() - pose2.tx()).pow(2)+(pose1.ty() - pose2.ty()).pow(2)+(pose1.tz() - pose2.tz()).pow(2))
        return distance
    }
    private fun getMidPose(startPose: Pose, endPose: Pose):Pose{
        var midX = (startPose.tx()+endPose.tx())/2
        var midY = (startPose.ty()+endPose.ty())/2
        var midZ = (startPose.tz()+endPose.tz())/2
